...
export const MessageInput: React.FC<MessageInputProps> = ({
  ...
  return <div className="p-4 bg-transparent">
      <div {...restRootProps} style={{
      display: 'flex',
      padding: '12px',
      ...
      backdropFilter: 'blur(6px)'
    }} className={`relative transition-colors ${isDragActive ? 'border-primary ring-2 ring-primary ring-offset-2' : ''} ${isLoading ? 'opacity-70' : ''}`}>
        <input {...inputProps} />
        
        {/* Main input area */}
        <div className="p-4">
          <Textarea value={message} onChange={e => onMessageChange(e.target.value)} placeholder={isDragActive ? "Drop files to attach" : "How can I help..."} className="w-full resize-none border-0 shadow-none focus-visible:ring-0 p-0 bg-transparent text-base placeholder:text-gray-400 min-h-[60px]" disabled={isLoading} onKeyPress={e => {
        ...
        <div className="flex items-center justify-between px-4 pb-4 gap-2">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
              <PopoverTrigger asChild>
                <Button variant="outline" size="icon" className="h-10 w-10 rounded-xl border-gray-200 bg-white flex-shrink-0" disabled={isLoading}>
                  <Plus className="h-4 w-4" />
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-48 p-2 bg-white border border-gray-200 shadow-lg z-50" align="start">
                <div className="space-y-1">
                  <Button variant="ghost" className="w-full justify-start" onClick={handleUploadFile} disabled={isLoading}>
                    Upload File
                  </Button>
                  <Button variant="ghost" className="w-full justify-start" onClick={handleAddUrl}>
                    Add URL
                  </Button>
                </div>
              </PopoverContent>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {/* Microphone button */}
            <Button variant="outline" size="icon" className="h-10 w-10 rounded-xl border-gray-200 bg-white" disabled={isLoading}>
              <Mic className="h-4 w-4" />
            </Button>

            {/* Send button */}
            <Button onClick={onSendMessage} disabled={isDisabled} size="icon" className="h-10 w-10 rounded-xl bg-gray-200 hover:bg-gray-300 text-gray-600">
        ...
        {isLoading && loadingStage && <div className="px-4 pb-2">
