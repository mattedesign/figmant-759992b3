...
export const SidebarMenuSection: React.FC<SidebarMenuSectionProps> = ({
  ...
  const renderMenuItem = (item: MenuItem) => <Button key={item.id} variant="ghost" className={cn("w-full justify-start text-left", activeSection === item.id ? "bg-white text-[#3D4A5C] rounded-[12px] shadow-[0px_1.25px_3px_0px_rgba(50,50,50,0.10),0px_1.25px_1px_0px_#FFF_inset]" : "hover:bg-white hover:text-[#3D4A5C] hover:rounded-[12px] hover:shadow-[0px_1.25px_3px_0px_rgba(50,50,50,0.10),0px_1.25px_1px_0px_#FFF_inset]")} style={{
    display: 'flex',
    height: '40px',
    ...
    if (activeSection !== item.id) {
      ...
      <div className={cn("w-8 h-8 mr-1 rounded-[8px] flex items-center justify-center", activeSection === item.id ? "bg-transparent" : "bg-[var(--Surface-03,#F1F1F1)]")}>
        <item.icon className={cn("h-4 w-4", activeSection === item.id ? "text-[#3D4A5C]" : "text-[#455468]")} />
      </div>
      <span className={cn("flex-1 text-left", activeSection === item.id ? "text-[#3D4A5C] font-semibold" : "text-[#455468] font-medium")} style={{
      overflow: 'hidden',
  ...
  const renderMenuSection = (section: MenuSection, sectionKey: string) => {
    ...
    return <div key={sectionKey} className="space-y-2">
        ...
        {(!section.title || !isCollapsed) && <div style={{
        display: 'flex',
        padding: '12px',
